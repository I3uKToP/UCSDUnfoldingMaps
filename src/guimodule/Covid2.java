...
public class Covid2 extends PApplet {
	...
	public void setup () {
		...
		CovidCount = loadLifeExpectancyFromCSV ("D:/AverangeWage3.csv");
	...
	private Map<String, Float> loadLifeExpectancyFromCSV(String fileName) {
		...
		for (String row : rows) {
			...
			lifeExpMap.put(columns[0], Float.parseFloat(columns[2]));
			}
	...
	private void shadeCountries () {
		for (Marker marker : countryMarkers) {
			...
			if (CovidCount.containsKey(countryID)) {
				...
				int colorLevel = (int) map (Covid, 1,130000, 10, 255);
